...
public class Punch : MonoBehaviour
{
    ...
    [SerializeField] private GameObject punchPrefab; // 이 스크립트가 붙은 프리팹 자신을 넣으셔도 됩니다.
    ...
    {
        if (Time.time >= lastAttackTime + punchData.cooldown)
        {
            ExecuteSlash();
    ...
    {
        if (punchPrefab == null) return;

        if (punchData.projectileCount <= 1)
        {
        ...
        {
            float angleStep = 15f;
            float startAngle = -((punchData.projectileCount - 1) * angleStep) / 2f;

            for (int i = 0; i < punchData.projectileCount; i++)
            {
    ...
    {
        GameObject projGO = Instantiate(punchPrefab, firePoint.position, rotation);
        Punch projScript = projGO.GetComponent<Punch>();

        if (projScript != null)
        {
            // 생성된 투사체의 모드를 변경하고 데이터를 주입합니다.
            projScript.isLauncher = false;
            projScript.currentDamage = punchData.damage;
            projScript.currentRange = punchData.range;

            // Rigidbody를 사용해 앞으로 즉시 발사합니다.
            Rigidbody rb = projGO.GetComponent<Rigidbody>();
            rb.useGravity = false; // 검기이므로 중력은 끕니다.
            rb.linearVelocity = projGO.transform.forward * punchData.speed;
            // ※ 유니티 구버전(2023 미만)을 쓰신다면 linearVelocity 대신 velocity를 사용하세요.
        }
    }
